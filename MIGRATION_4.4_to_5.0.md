...
other providers later won't be another breaking change — and 5.0 already does, with Anthropic
alongside OpenAI.
...
- Dependency changes:
  ...
  - `Microsoft.Extensions.DependencyInjection.Abstractions` → **10.0.11** (was transitive, now explicit)
  - `Microsoft.Extensions.Logging.Abstractions` → **10.0.11** (was 10.0.9)
  - `OpenAI` → **2.13.0** (new — the official SDK, replacing `Betalgo.Ranul.OpenAI`, which is gone)
  - `Anthropic` → **12.40.0** (new)
  - `Scriban` → **7.2.6** (was 7.2.5)
  - `Microsoft.Bcl.Memory` → **10.0.11** (new, and a pin rather than a feature —
    `Microsoft.ML.Tokenizers.Data.O200kBase` declares only a netstandard2.0 dependency group, so
    even on .NET 10 it asks for the 9.0.4 that carries GHSA-73j8-2gch-69rq. The pin goes away once
    that package ships against a patched version.)
  - `Betalgo.Ranul.OpenAI` → **removed**, replaced by `OpenAI` above.
  - `TypeMerger` → **removed**. The template engine builds its own render context now.
...
This goes further: `ChatAsync` works on a copy, so **nothing** is written back onto your options.
...
Structured output, code execution and file I/O all work on both providers, from the same
`ChatOptions` — the schema is generated by Ouroboros rather than taken from a vendor SDK,
which is what lets one `ResponseType` serve either.

### 7. Server-side code execution and file I/O
...
### 7c. Paused turns are continued for you
...
### 8. Responses carry typed content blocks and a stop reason
